...
def load_conllu(file):
    ...
    while True:
        ...
        # ud start is none in the beginning so come here
        # this should happen only at the very beginning
        if sentence_start is None:
            ...
            # sentence start will keep going up
            sentence_start = len(ud.words)
        # NOTE that line can become null here after stripping newline
        # characters; that happens at the end of every sentence b/c
        # newline characters separate the sentences
        if not line:
            # comes in here if we have reached end of sentence
            # Add parent and children UDWord links and check there are no cycles
            def process_word(word):
                # goal: adding parent relationships to the UDWords
                ...
                if word.parent is None:
                    ...
                    if head:
                        # parent word
                        # head is just a number so add the head number; jump
                        # to that position
                        parent = ud.words[sentence_start + head - 1]
                        word.parent = "remapping"
                        # call it on the parent and then find the parent of the
                        # parent which hasn't been set yet
                        process_word(parent)
                        word.parent = parent

            # you are always working at the end so makes sense to
            # go from sentence_start to end; this for loop will
            # create the child-parent relationships in the file
            ...
            for word in ud.words[sentence_start:]:
                # if the word has a parent and the word has a deprel in the
                # functional deprel category list above; then this is a
                # functional child of that parent
                if word.parent and word.is_functional_deprel:
                    word.parent.functional_children.append(word)

            # Check there is a single root node
            # there should only be one node whose parent is null
        ...
        # index is position of last token processed
        index += len(columns[FORM])

        # Handle multi-word tokens to save word(s)
        if "-" in columns[ID]:
            raise UDError("not today")
            try:
...
def evaluate(gold_ud, system_ud):
    ...
    def spans_score(gold_spans, system_spans):
        # print("gold:")
        # [print(f"({g.start} {g.end})") for g in gold_spans]
        # print("system:")
        # [print(f"({g.start} {g.end})") for g in system_spans]
        correct, gi, si = 0, 0, 0
        while gi < len(gold_spans) and si < len(system_spans):
            if system_spans[si].start < gold_spans[gi].start:
                ...
                # the start, end tuple of both gold and system
                # are exactly same; so we mark it correct
    ...
    def align_words(gold_words, system_words):
        ...
        # just gather the parts where the FORM parts in the gold
        # and system files agree with each other
        ...
        while gi < len(gold_words) and si < len(system_words):
            if gold_words[gi].is_multiword or system_words[si].is_multiword:
                ...
                if (gold_words[gi].span.start, gold_words[gi].span.end) == (system_words[si].span.start, system_words[si].span.end):
                    ...
                    # start is same but ends are not the same
                    # add to gi so gi can catch up
                    gi += 1
                else:
                    # gold's start is bigger now
                    # otherwise add to si so si can catch up
                    si += 1
    ...
    return {
        # for our use case, it should be the case that tokens and words
        # will have same results
        # we do the comparison only on the aligned nodes
...
def my_evaluate_wrapper(gold_file, system_file):
    # Load CoNLL-U files
    ...
    out.append("Metric     | Precision |    Recall |  F1 Score | AligndAcc")
    out.append("-----------+-----------+-----------+-----------+-----------")
    for metric in ["Tokens", "Sentences", "Words", "UPOS", "XPOS", "UFeats", "AllTags", "Lemmas", "UAS", "LAS", "CLAS", "MLAS", "BLEX"]:
        out.append("{:11}|{:10.2f} |{:10.2f} |{:10.2f} |{}".format(
            metric,
